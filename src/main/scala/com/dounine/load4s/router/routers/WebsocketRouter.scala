package com.dounine.load4s.router.routers
...
import com.dounine.load4s.behaviors.client.SocketBehavior
...
class WebsocketRouter(system: ActorSystem[_]) extends SuportRouter {
  ...
  val route: Route = concat(
    get {
      path(pm = "ws" / "udp") {
        handleWebSocketMessages(createConnect())
      }
    },
    get {
      getFromResourceDirectory(s"dist")
    },
    get {
      path("") {
  ...
  private def createConnect(): Flow[Message, Message, _] = {
    val uuid: String = UUID.randomUUID().toString.replaceAll("-", "")

    val socketBehavior: ActorRef[BaseSerializer] =
      system.systemActorOf(SocketBehavior(), s"socket_${uuid}")
